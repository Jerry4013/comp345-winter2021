...
Player *BidingFacility::bid(vector<Player *> players) {
    int bidding;
    int highestBidding = -1;
    vector<Player*> highestPlayers;
    for (int i = 0; i < players.size(); ++i) {
        cout << "Please enter the bidding of player " << players[i]->getId() << "(" <<
        players[i]->getLastName() << "): ";
        cin >> bidding;
        players[i]->setBidding(bidding);
    ...
    for (int i = 1; i < highestPlayers.size(); ++i) {
        if (highestPlayers[i]->getLastName() < lastName) {
            ...
            alphabeticalLastNamePlayers.emplace_back(players[i]);
        } else if (highestPlayers[i]->getLastName() == lastName) {
            alphabeticalLastNamePlayers.emplace_back(players[i]);
        }
    }
    if (alphabeticalLastNamePlayers.size() == 1) {
        return alphabeticalLastNamePlayers[0];
    }
